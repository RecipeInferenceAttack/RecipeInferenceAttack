...
def writeTxt(list, name, runNum):
    path = 'demo2/' + str(runNum)
    if not os.path.exists(path):
        os.makedirs(path)
    path = 'demo2/' + str(runNum) + '/npy'
    if not os.path.exists(path):
        os.makedirs(path)
    filename = open('demo2/' + str(runNum) + '/' + name + '.txt', 'w')
    for value in list:
        filename.write(str(value))
        filename.write('\n')
    filename.close()
    list = np.array(list)
    np.save('demo2/' + str(runNum) + '/npy/' + name + '.npy', list)
...
def main(runNum):
    ...
    print('randomDeviceList:', randomDeviceList)
    writeTxt(randomDeviceList, 'randomDeviceList', runNum)
